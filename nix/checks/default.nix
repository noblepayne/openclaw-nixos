{
  ...
  systemModuleEval = nixpkgs.lib.nixosSystem {
    modules = [
      {
  ...
  userModuleEval = nixpkgs.lib.nixosSystem {
    modules = [
      {
        nixpkgs.hostPlatform = system;
        system.stateVersion = "26.05";
  ...
  syntaxCheck =
    pkgs.runCommand "openclaw-script-syntax-check" {
      nativeBuildInputs = [
        pkgs.bash
        pkgs.nodejs
      ];
  ...
  profileModuleEval = nixpkgs.lib.nixosSystem {
    modules = [
      {
        nixpkgs.hostPlatform = system;
        system.stateVersion = "26.05";
...
in ({
    lib-render-plugins = assert renderedPluginsConfig.plugins.allow
    == [
      "manual-plugin"
      "telegram"
    ...
    local-plugin-package-basic =
      pkgs.runCommand "openclaw-local-plugin-basic-check" {
    ...
    local-plugin-package-runtime-deps =
      pkgs.runCommand "openclaw-local-plugin-runtime-deps-check" {
        nativeBuildInputs = [pkgs.jq];
      } ''
    ...
    module-eval-system-service = assert systemModuleEval.config.systemd.services.openclaw.serviceConfig.ExecStart
    ...
    module-eval-user-service = assert userModuleEval.config.systemd.user.services.openclaw.unitConfig.ConditionUser == "chris";
    ...
    module-eval-plugin-profile = assert profileModuleEval.config.services.openclaw.bundledPlugins.telegram.enable;
    ...
    module-negative-assertions = assert !invalidSystemStateEval.success;
